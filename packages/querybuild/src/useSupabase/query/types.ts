// Tipo para validar que Tables o Views existan
export type ValidatedSchema<
    SchemaKey extends keyof D,
...
> = D[SchemaKey] extends {
    ...
    ? T extends Record<string, any>
        ? T
        : never
    : D[SchemaKey] extends { Views: infer V }
      ? V extends Record<string, any>
          ? V
          : never
      : never;

export type TableRow<Schema extends BaseSchema, TableName extends keyof Schema['Tables']> =
    Schema['Tables'] extends Record<string, { Row: any }>
        ? TableName extends keyof NonNullable<Schema['Tables']>
            ? NonNullable<Schema['Tables']>[TableName]['Row']
            : never
        : never;

export type ViewRow<Schema extends BaseSchema, ViewName extends keyof Schema['Views']> =
    Schema['Views'] extends Record<string, { Row: any }>
        ? ViewName extends keyof NonNullable<Schema['Views']>
            ? NonNullable<Schema['Views']>[ViewName]['Row']
            : never
        : never;
