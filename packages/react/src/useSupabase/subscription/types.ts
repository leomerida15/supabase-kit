...
import { DatabaseTemp, TableRow, ViewRow } from '../query';
...
export interface PayloadRealtime<
    D extends DatabaseTemp,
    SchemaKey extends keyof D,
    K extends keyof (D[SchemaKey]['Tables'] &
        D[SchemaKey]['Views']) = keyof (D[SchemaKey]['Tables'] & D[SchemaKey]['Views']) &
        Parameters<SupabaseClient<D>['from']>['0'],
    V = (D[SchemaKey]['Tables'] & D[SchemaKey]['Views'])[K] extends {
        Row: infer R;
    }
        ? R extends Array<any>
            ? keyof R[0]
